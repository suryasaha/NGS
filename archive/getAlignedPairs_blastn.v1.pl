...
getAlignedPairs_blastn.v1.pl - Get the pairs which align to the reference genome with correct orientation
 
=head1 SYNOPSIS

getAlignedPairs.v1.pl -blastnrep blastn.out -pairedreads file -out file -outformat Fasta/fastq
...
Version 1: Only pulls out reads that match to wolbachia genomes. Handles only innies, i.e. short insert 
PE libs or revcomped MP libs without any PE contamination.
BUG: Counting space between end of fwd and end of rev instead of end of fwd and start of rev
...
@temp1,$minins,$maxins,$ctr,%genomes,$debug);
			...
			if($genomes{$i} eq 'w'){#if hit is a wolbachia species
				while($hsp = $hit->next_hsp()){# $hsp is Bio::Search::HSP::HSPI compliant object
					if($hsp->length('query') >= $mlen){#if length of read involved in HSP > match length
						#record name, ref genome strand/end/
						#recording end since the inslen for each lib does not include read lengths
						push @temp,$hit->name(); push @temp,$hsp->strand('subject');
						push @temp,$hsp->end('subject');
					}
				}
			}
	    }
	    #debug
		if($debug){foreach $i (0..$#temp){if(($i%3==0)&&($i!=0)){print STDERR "\n"; } print STDERR "\t$temp[$i]";}}
	    #store in hash
...
my(%qualwreads,%qualdcereads,$fwd,@fwdarr,$rev,@revarr);
...
while(($i,$j)=each(%blastnwreads)){
	if($i=~ /1$/){#forward read
		...
		if(exists $blastnwreads{$i}){# get rev read data
			...
			for($i=0;$i<@fwdarr;$i+=3){
				for($j=0;$j<@revarr;$j+=3){
					#if hit on same genome and diff strand and within range
					if(($fwdarr[$i] eq $revarr[$j]) && ($fwdarr[$i+1] != $revarr[$j+1])){ 
						#BUG counting space between end of fwd and end of rev instead of end of fwd and start of rev
						#fwd on pos and rev on comp strand
						if (($fwdarr[$i+1] == 1) && ($revarr[$j+1] == -1) && ($fwdarr[$i+2]+$minins <= $revarr[$j+2]) 
							&& ($fwdarr[$i+2]+$maxins >= $revarr[$j+2])){
							$qualwreads{$fwd}=$fwdarr[$i];#record fwd read name and genome it aligned to
							$qualwreads{$rev}=$revarr[$j+2]-$fwdarr[$i+2];# rev read name and insert length
							if($debug){print STDERR "\tSelected Fwd: $fwd and Rev: $rev with insert length: $qualwreads{$rev}\n";}
							$ctr++; if($ctr%1000 == 0){print STDERR "$ctr..";} goto OUT1;#done for this pair so get out
						}
						#fwd on neg and rev on pos strand
						elsif (($fwdarr[$i+1] == -1) && ($revarr[$j+1] == 1) && ($revarr[$j+2]+$minins <= $fwdarr[$i+2]) 
							&& ($revarr[$j+2]+$maxins >= $fwdarr[$i+2])){
							$qualwreads{$fwd}=$fwdarr[$i];#record fwd read name and genome it aligned to
							$qualwreads{$rev}=$fwdarr[$i+2]-$revarr[$j+2];# rev read name and insert length
							if($debug){print STDERR "\tSelected Fwd: $fwd and Rev: $rev with insert length: $qualwreads{$rev}\n";}
							$ctr++; if($ctr%1000 == 0){print STDERR "$ctr..";} goto OUT1;#done for this pair so get out
						}
						else{
							if($debug){print STDERR "\tRejected Fwd: $fwd and Rev: $rev due to \n\t\tinsert length: ",
									$fwdarr[$i+2]-$revarr[$j+2],"\tFwd strand: ",$fwdarr[$i+1],"\tRev strand: ",$revarr[$j+1],"\n";}
						}
					}
				}
			}
...
$i=$ctr=0; print STDERR "Writing qualifying and unmapped reads..";
